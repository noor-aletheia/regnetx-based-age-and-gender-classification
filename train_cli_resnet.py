...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description='ResNet Age & Gender Classification Training',
...
def determine_models_to_train(args, config: Config) -> List[str]:
    if args.model:
        available_models = ResNetModelFactory.SUPPORTED_MODELS
        ...
        return config.get('models.variants', ['resnet34', 'resnet50'])
...
def print_training_summary(config: Config, models_to_train: List[str]):
    ...
    logger.info(f"Device: {config.get('hardware.device')}")
        # Augmentation summary logic
    # Augmentation summary logic (check only augmentation.mode)
    # aug_keys = [
    #     ('motion_blur', config.get('augmentation.mode.motion_blur.prob', 0)),
    #     ('random_affine', config.get('augmentation.mode.random_affine.degrees', 0)),
    #     ('color_jitter_brightness', config.get('augmentation.mode.color_jitter.brightness', 0)),
    #     ('color_jitter_contrast', config.get('augmentation.mode.color_jitter.contrast', 0)),
    #     ('color_jitter_saturation', config.get('augmentation.mode.color_jitter.saturation', 0)),
    #     ('color_jitter_hue', config.get('augmentation.mode.color_jitter.hue', 0)),
    #     ('horizontal_flip', config.get('augmentation.mode.horizontal_flip', 0)),
    #     ('sharpening', config.get('augmentation.mode.sharpening.prob', 0)),
    #     ('clahe', config.get('augmentation.mode.clahe.prob', 0)),
    # ]
    # enabled_augs = [name for name, val in aug_keys if val and val > 0]
    # if enabled_augs:
    #     logger.info(f"Augmentation: Enabled ({', '.join(enabled_augs)})")
    # else:
...
def main():
    ...
    for i, model_name in enumerate(models_to_train, 1):
        ...
        logger.info(f"{'='*60}")
        try:
            from src.train_resnet import train_single_model_resnet
            result = train_single_model_resnet(
